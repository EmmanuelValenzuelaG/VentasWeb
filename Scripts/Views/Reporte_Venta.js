...
$('#btnBuscar').on('click', function () {
    ...
    jQuery.ajax({
        url: $.MisUrls.url._ObtenerReporteVenta + "?fechainicio=" + $("#txtFechaInicio").val() + "&fechafin=" + $("#txtFechaFin").val() + "&idtienda=" + $("#cboTienda").val() + "&tipodedocumento=" + $("#cboventatipodocumento").val(),
        type: "GET",
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        success: function (data) {
...
function exportExcel() {
    ...
    if ($('#tbReporte tbody tr').length == 0) {
        ...
        document.getElementById('dowloadexcel').addEventListener('click', function () {
            ...
            table2excel.export(document.querySelectorAll("#tbReporte"));})
    }
}
